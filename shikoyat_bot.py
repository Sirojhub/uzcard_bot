...
# python-telegram-bot v13.15 imports
...
# ========== CONFIG ==========
TOKEN = "8530769366:AAHsxprn6KJwOzLP4D7Z-0KZwyL9EkPoFI0"   # <-- Bu yerga to'liq tokeningizni qo'ying
ADMIN_ID = 1052080030            # <-- Bu yerga sizning numeric Telegram ID (raqam) ni qo'ying
...
AUTO_ADS_INTERVAL = 86400  # 24 soat (sekundlarda) - o'zgartiring kerak bo'lsa
...
def allowed_action(user_id):
    ...
    # faqat so'nggi SPAM_LIMIT_SECONDS ichidagi harakatlarni saqlaymiz
...
cards_info = {
    "uzcard_unionpay": {
        "title": "💳 Uzcard + UnionPay",
        "text": "1️⃣ Uzcard + UnionPay\nNarxi: 40 000 so‘m\nMuddati: 3 yil\nValyuta: So‘m",
        "image": None,
    },
    "humo_visa": {
        "title": "💳 Humo + Visa",
    ...
    "visa": {
        "title": "💳 Visa karta",
        "text": "3️⃣ Visa karta\nNarxi: 25 000 so‘m\nValyuta: Faqat dollar tushadi",
        "image": None,
    },
    "uzcard_kids": {
        "title": "👶 Uzcard Kids karta",
        "text": "4️⃣ Uzcard Kids karta\nBolalar uchun maxsus",
        "image": None,
    },
}

# misol filiallar — istasangiz o'zgartiring (Surxondaryo filiallari kiritilishi mumkin)
bank_branches = [
    {
        "name": "Markaziy Filial",
        "address": "Toshkent, Mustaqillik 1",
        "map": "https://maps.google.com/?q=Toshkent+Mustaqillik+1",
    },
    {"name": "Shahar Filiali", "address": "Toshkent, Ko'ch 5", "map": "https://maps.google.com/?q=Toshkent+Koch+5"},
]
...
def main_menu():
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("🤝 Botga qo‘shilish", url="https://t.me/ariza_bot2921_bot"),
        ],
    ]
...
def start(update: Update, context: CallbackContext):
    ...
    # agar background rasm ishlatmoqchi bo'lsangiz, "background.jpg" faylini joylashtiring:
    # agar yo'q bo'lsa, oddiy text bilan yuboring
    try:
        bg_path = "background.jpg"
        if os.path.exists(bg_path):
            # rasm yuborish bilan menyu
            context.bot.send_photo(
                chat_id=update.message.chat_id,
                photo=open(bg_path, "rb"),
                caption="Kartalar ro‘yxatiga xush kelibsiz! Birini tanlang:",
                reply_markup=main_menu(),
            )
            return
    except Exception as e:
        logger.warning("Background rasm yuborishda xato: %s", e)
...
def callback_button(update: Update, context: CallbackContext):
    ...
    # har doim answer qilishni unutmang (push notifications to remove "loading")
    try:
        query.answer()
    except Exception:
        pass
    ...
    if data in cards_info:
        ...
        try:
            if card["image"]:
                context.bot.send_photo(
                    chat_id=query.message.chat.id, photo=card["image"], caption=card["text"], reply_markup=back_button()
                )
            ...
            # agar edit_message yoki send_photo muammo bersa, fallback
            try:
                query.message.reply_text(card["text"], reply_markup=back_button())
            except Exception:
                logger.exception("Card show error")
        return
    ...
    if data == "back":
        try:
            ...
            try:
                ...
                logger.exception("Back button error")
        return
    ...
    if data == "bank_branches":
        ...
        try:
            ...
            try:
                ...
                logger.exception("bank_branches error")
        return
    ...
    if data == "stats":
        txt = (
            ...
            f"📂 Menyu ochilgan: *{stats.get('menu_clicks', 0)} marta*\n"
            f"💳 Kartalar ko‘rilgan: *{stats.get('card_views', 0)} marta*\n"
            f"📢 Auto-reklama: *{'ON' if stats.get('auto_ads') else 'OFF'}*"
        )
        try:
            query.edit_message_text(txt, parse_mode=ParseMode.MARKDOWN, reply_markup=back_button())
        except Exception:
    ...
    if data == "apply_card":
        try:
            ...
            try:
                ...
                logger.exception("apply_card error")
        return
...
def apply_passport(update: Update, context: CallbackContext):
    ...
    summary = (
        ...
        f"Familiya: {context.user_data.get('app_family')}\n"
        f"Telefon: {context.user_data.get('app_phone')}\n"
        f"Passport: {context.user_data.get('app_passport')}\n\n"
...
def apply_confirm(update: Update, context: CallbackContext):
    ...
    if txt in ("ha", "ok"):
        app = {
            ...
            "family": context.user_data.get("app_family"),
        ...
        try:
            msg = (
                f"🔔 Yangi ariza:\nID: {app['user_id']}\nIsm: {app['name']}\n"
                f"Familiya: {app['family']}\nTelefon: {app['phone']}\nPassport: {app['passport']}\nVaqti: {app['time']}"
            )
            context.bot.send_message(chat_id=ADMIN_ID, text=msg)
        except Exception:
            logger.exception("Admin ga ariza yuborishda xato")
    ...
    if data == "admin_users":
        ...
        try:
            ...
            try:
                ...
                logger.exception("admin_users error")
        return
        ...
        if not apps:
            try:
                ...
                try:
                    query.message.reply_text("Ariza yo'q.")
                except Exception:
                    logger.exception("admin_apps error")
            return
        else:
            lines = []
            for a in apps[-50:]:
                lines.append(
                    f"ID:{a['user_id']} | {a['name']} {a['family']} | {a['phone']} | {a['passport']} | {a['time']}"
                )
            try:
                query.edit_message_text("🔔 So‘nggi arizalar:\n\n" + "\n".join(lines))
            except Exception:
                try:
                    query.message.reply_text("🔔 So‘nggi arizalar:\n\n" + "\n".join(lines))
                except Exception:
                    logger.exception("admin_apps error")
            return
        ...
        try:
            ...
            try:
                query.message.reply_text(f"Auto-reklama: {'ON' if stats['auto_ads'] else 'OFF'}")
            except Exception:
                logger.exception("admin_toggle_ads error")
        return

    elif data == "admin_broadcast":
        # admin xabar yuborish uchun modalitet: admin /admin tugmasini bosganda text yuboradi
        try:
            query.edit_message_text("Iltimos, yuboriladigan matnni yozing. (Esc: /cancel)")
        except Exception:
            try:
                query.message.reply_text("Iltimos, yuboriladigan matnni yozing. (Esc: /cancel)")
            except Exception:
                logger.exception("admin_broadcast error")
        # belgini saqlash - admin_text_handler ichida ishlatiladi
        ...
        try:
            ...
            try:
                query.message.reply_text("Admin paneldan chiqildi.")
            except Exception:
                logger.exception("admin back error")
        return
        ...
        try:
            ...
            try:
                query.message.reply_text("Noma'lum amr.")
            except Exception:
                logger.exception("admin unknown command")
        return
    ...
    if context.user_data.get("awaiting_broadcast"):
        ...
        for uid in users:
            try:
                ...
                # agar yuborib bo'lmasa o'tkazib yuboramiz
                continue
    ...
    if stats.get("auto_ads"):
        ...
        for uid in users:
            try:
                ...
                # foydalanuvchi bloklagan yoki xato bo'lsa, log yozamiz va davom etamiz
                logger.warning("Auto-ads: xabar yuborilmadi %s: %s", uid, e)
                continue
...
def main():
    ...
    # /admin command and admin text handler
    dp.add_handler(CommandHandler("admin", admin_command))
    dp.add_handler(MessageHandler(Filters.text & ~Filters.command, admin_text_handler))

    # 🔹 Admin callback handler - qo'shing AVVAL (general callbackdan oldin)
    dp.add_handler(CallbackQueryHandler(admin_callback, pattern="^admin_"))

    # Callback handler for general buttons (cards, back, stats, etc.)
    ...
    # Start auto-ads thread (daemon)
    def auto_ads_loop():
        while True:
            try:
                auto_ads_worker(updater)
            except Exception as e:
                logger.exception("auto_ads_loop ichida xato: %s", e)
    ...
    logger.info("Bot ishga tushdi. Polling ishlayapti.")
    updater.idle()
